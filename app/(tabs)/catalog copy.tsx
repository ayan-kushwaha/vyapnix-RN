...
import { deleteItem, cloneTemplate } from '@/src/store/catalogSlice'
  ...
  switch (view.mode) {
    case 'chooseTemplate':
      return <ChooseTemplateView
        ...
        // ✅ FIX: 'onNavigateToUpdate' function ko yahan se pass kiya gaya hai
    ...
    case 'updateTemplate':
      ...
      return (
        <View style={tw`flex-1 justify-center items-center p-4`}>
          <Text style={tw`text-lg text-gray-400 text-center font-bold`}>An update is available for "{view.template.templateName}".</Text>
          <Text style={tw`text-center my-2 text-gray-500`}>
            Updating will add new fields from the starter template to your cloned version. Your custom fields will not be affected. Are you sure?
          </Text>
          <TouchableOpacity onPress={() => { /* TODO: Dispatch update action */ Alert.alert("Update logic coming soon!"); }} style={tw`bg-green-500 p-3 rounded-lg mt-4 w-full items-center`}>
            <Text style={tw`text-white font-bold`}>Yes, Update Now</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setView({ mode: 'list' })} style={tw`mt-2 p-2 w-full items-center`}>
    ...
    // ... baaki saare cases waise hi rahenge ...
    ...
    case 'itemDetail':
      ...
      return <ItemDetailView item={view.item} template={templateForItemDetail} onClose={() => setView({ mode: 'list' })} onEdit={() => setView({ mode: 'editItem', item: view.item })} onDelete={() => { Alert.alert("Delete Item", `Are you sure?`, [{ text: "Cancel" }, { text: "Delete", style: "destructive", onPress: () => { dispatch(deleteItem(view.item!._id)); setView({ mode: 'list' }) } }]) }} />
    case 'editTemplate':
      if (!view.template) return null
    ...
    case 'editItem':
      ...
      if (!templateForItem) { return <View style={tw`flex-1 justify-center items-center`}><Text>Error: Could not find matching template.</Text></View> }
    ...
    default:
      return (
        <CatalogListView
          ...
          onNavigateToUpdateTemplate={(template) => setView({ mode: 'updateTemplate', template })} // ✅ FIX: Yeh prop yahan se pass ho raha hai
