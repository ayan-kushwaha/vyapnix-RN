...
import { CustomInput, SwitchInput, DateTimeInput, CheckboxInput, SearchableDropdown } from '@/src/components/forms/FormUI';
...
export const AddItemForm: React.FC<AddItemFormProps> = ({ onClose, template, itemToEdit }) => {
    ...
    const handleSaveItem = async () => {
        ...
        const payload: Partial<CatalogItem> = {
            template: template._id, name: formData.name, description: formData.description,
            pricingOptions: [{ label: "Standard Price", basePrice: Number(formData.basePrice), cycle: 'one-time', taxAmount: 0, totalPrice: 0 }],
            stock: Number(formData.stock), images: formData.images, tags: tagsArray, isActive: formData.isActive,
            dynamicFields: formData.dynamicFields, tax: formData.taxId || undefined,
        };
        try {
    ...
    // ✅ NEW: Dynamic Field Renderer function
    const renderDynamicField = (field: ItemTemplate['fields'][0]) => {
        ...
        switch (field.fieldType) {
            ...
            case 'dropdown-single': return <SearchableDropdown key={fieldName} label={field.label} data={(field.options || []).map((o: string) => ({ label: o, value: o }))} value={value} onSelectionChange={(v: string | string[]) => handleDynamicFieldChange(fieldName, v)} />;
            case 'dropdown-multi': return <SearchableDropdown key={fieldName} label={field.label} mode="multiple" data={(field.options || []).map((o: string) => ({ label: o, value: o }))} value={value || []} onSelectionChange={(v: string | string[]) => handleDynamicFieldChange(fieldName, v)} />;
    ...
    return (
        <SafeAreaView style={[tw`flex-1 `, { backgroundColor: theme.colors.background }]}>
            ...
            <ScrollView contentContainerStyle={tw`p-6 pb-20`} keyboardShouldPersistTaps="handled">
                ...
                <View style={[tw`p-4 rounded-lg my-4`, { backgroundColor: theme.colors.card, borderColor: theme.colors.border, borderWidth: 1 }]}><View style={tw`flex-row justify-between items-center`}><View style={tw`flex-1 mr-4`}><View style={tw`flex-row items-center`}>{formData.isActive ? <Globe size={18} color={theme.colors.secondary as string} /> : <Lock size={18} color={theme.colors.textSecondary as string} />}<Text style={[tw`font-bold ml-2`, { color: theme.colors.text }]}>{formData.isActive ? t.status.publicLabel : t.status.privateLabel}</Text></View><Text style={[tw`text-xs mt-1`, { color: theme.colors.textSecondary }]}>{formData.isActive ? t.status.publicDesc : t.status.privateDesc}</Text></View><Switch trackColor={{ false: theme.colors.border as string, true: theme.colors.secondary as string }} thumbColor={"white"} onValueChange={v => handleInputChange('isActive', v)} value={formData.isActive} /></View></View>
                <View style={[tw`p-4 pb-4  rounded-lg`, { backgroundColor: theme.colors.card, borderColor: theme.colors.border, borderWidth: 1 }]}>
                    <CustomInput label={t.price} icon={IndianRupee} value={String(formData.basePrice)} onChangeText={v => handleInputChange('basePrice', v)} keyboardType="numeric" /><SearchableDropdown label="Tax Rate" data={taxOptions as any} value={formData.taxId || ''} onSelectionChange={v => handleInputChange('taxId', v as string | null)} /><View style={[tw`mt-4 pt-4 border-t`, { borderColor: theme.colors.border }]}><View style={tw`flex-row justify-between mb-1`}><Text style={{ color: theme.colors.textSecondary }}>Base Price</Text><Text style={{ color: theme.colors.textSecondary }}>₹{priceDetails.base.toFixed(2)}</Text></View><View style={tw`flex-row justify-between mb-1`}><Text style={{ color: theme.colors.textSecondary }}>{priceDetails.taxName}</Text><Text style={{ color: theme.colors.textSecondary }}>+ ₹{priceDetails.taxAmount.toFixed(2)}</Text></View><View style={[tw`flex-row justify-between mt-2 pt-2 border-t`, { borderColor: theme.colors.border }]}><Text style={[tw`font-bold`, { color: theme.colors.text }]}>Total Price</Text><Text style={[tw`font-bold`, { color: theme.colors.text }]}>₹{priceDetails.total.toFixed(2)}</Text></View></View></View>
                {template.modelType === 'e-commerce' && (
                    <View style={[tw`mt-4`]}>
                ...
                <TouchableOpacity onPress={handleSaveItem} disabled={isLoading} style={[tw`mt-8 w-full h-14 rounded-md items-center justify-center`, { backgroundColor: theme.colors.primary as string, opacity: isLoading ? 0.7 : 1 }]}>{isLoading ? <ActivityIndicator color="#fff" /> : <Text style={tw`text-white text-md e-full font-bold`}>{itemToEdit ? t.updateButton : t.saveButton}</Text>}</TouchableOpacity>
            </ScrollView>
        </SafeAreaView>
    );
};
